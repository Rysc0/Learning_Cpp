...
list *insert_at_head(list *head, int value, int index){
    ...
    // temp->next = head;
    // head = temp; 
    // temp->next->previous = NULL; // not sure here
    return head;
}
...
void insert_at_the_end(int value, int index, list *tail){
    ...
    ptr->next = NULL;
    ptr->previous = tail; // double check this
    tail = ptr;
}
...
void insert_inbetween(int value, int index, int index2, list *head){
    ...
    temp->next = ins->next;
    temp->next->previous = ins; // double check this
    ins->next = temp;
}
...
void list_display_end(list *tail){
    cout << "--- Displaying linked list ---" << endl;
    int i = 0;
    while(tail != NULL){
        cout << tail->value << "\t" << tail->index << endl;
        tail = tail->previous;  
        i++;
    }
    cout << "List has " << i << " nodes..." << endl;
...
int main(){
    ...
    cout << "Displaying from end to start" << endl;
    list_display_end(tail);
    cout << "------------------------------" << endl;
